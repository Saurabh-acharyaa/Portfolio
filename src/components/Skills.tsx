...
const SkillCard: FC<SkillCardProps> = ({ name, icon, color, index, controls, onClick }) => {
  ...
  return (
    <motion.div
      ...
      className="p-4 bg-gray-800/50 rounded-xl border border-gray-700 flex flex-col items-center transition-all duration-300 hover:border-gray-600 hover:bg-gray-800/70 group relative overflow-hidden cursor-pointer"
    ...
    >
      ...
      <motion.div
        ...
        className={`relative z-10 w-12 h-12 rounded-lg bg-gray-700/80 flex items-center justify-center text-2xl mb-3 group-hover:shadow-[0_0_20px_5px] group-hover:shadow-${glowColor}/30`}
      >
        {icon}
      </motion.div>
      
      <h3 className="relative z-10 text-sm font-medium text-gray-200 group-hover:text-white transition-colors duration-300">
        {name}
      </h3>
    </motion.div>
  );
...
const Skills: FC = () => {
  ...
  return (
    <section 
      id="skills" 
      ref={ref}
      className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 px-4 py-16 text-white relative"
    >
        ...
        >
          <div className="relative mb-4 group">
            <motion.div
              ...
              className="w-16 h-16 rounded-xl bg-gradient-to-br from-purple-600 to-purple-500 flex items-center justify-center shadow-lg group-hover:shadow-purple-500/30"
            >
            ...
            <motion.div
              ...
              className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-gradient-to-br from-cyan-400 to-cyan-300 border-4 border-gray-900 shadow-sm"
            ></motion.div>
          </div>
          ...
          <motion.h2
            ...
            className="text-3xl font-bold text-center bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-cyan-300 mb-2"
          >
            My Skills
          </motion.h2>
          <motion.p
            ...
            className="text-center text-gray-300 text-sm max-w-md"
          >
          ...
          <AnimatePresence>
            {selectedSkill && (
              <motion.div
                ...
                className="absolute z-50 w-72 bg-gradient-to-b from-gray-800 to-gray-900 rounded-xl overflow-hidden border border-gray-700 shadow-2xl"
                style={{
                  left: `${modalPosition.x}px`,
                  top: modalPosition.direction === 'bottom' ? 
                    `${modalPosition.y}px` : 
                    `${modalPosition.y - 240}px`,
                  transform: 'translateX(-50%)',
                  marginTop: modalPosition.direction === 'bottom' ? '15px' : undefined,
                  marginBottom: modalPosition.direction === 'top' ? '15px' : undefined
                }}
              >
                {/* Premium accent line */}
                ...
                <div className="relative p-5">
                  <button
                    onClick={closeModal}
                    className="absolute top-3 right-3 text-gray-400 hover:text-white transition-colors duration-200 z-10 p-1 rounded-full bg-gray-700/50 hover:bg-gray-700"
                  >
                    <FaTimes className="w-3.5 h-3.5" />
                  </button>
                  
                  <div className="flex flex-col items-center text-center">
                    <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-600 to-cyan-500 flex items-center justify-center text-2xl mb-4 shadow-lg">
                      {skills.find(s => s.name === selectedSkill.name)?.icon}
                    </div>
                    
                    <h2 className="text-xl font-bold text-white mb-3 bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-cyan-300">
                      {selectedSkill.name}
                    </h2>
                    
                    <div className="h-px w-16 bg-gradient-to-r from-transparent via-purple-500 to-transparent mb-4"></div>
                      ...
                      <p className="text-gray-300 text-sm leading-relaxed font-light">
                        ...
                        {!typingComplete && (
                          <motion.span
                            ...
                            className="ml-0.5 inline-block w-1 h-4 bg-cyan-400 align-middle"
                          />
                        )}
                      </p>
                    </div>
                ...
                {/* Arrow pointing to the skill card */}
                <div className={`absolute left-1/2 transform -translate-x-1/2 ${
                  modalPosition.direction === 'bottom' 
                    ? 'top-0 -translate-y-full' 
                    : 'bottom-0 translate-y-full'
                }`}>
                  <div className={`w-4 h-4 bg-gray-800 rotate-45 transform ${
                    modalPosition.direction === 'bottom' 
